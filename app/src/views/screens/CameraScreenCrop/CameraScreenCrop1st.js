...
function CameraScreen({route, navigation}) {
  ...
  const {
    // cameraType,
     categoryId, path, index, defaultCategory, myCategory} =
    route.params;
...
const refInput = React.useRef(null);
  ...
  const [categoryName, setCategoryName] = useState('Select/Add Folder');
  const [subCategory, setSubCategory] = useState('');
  const [subCategoryName, setSubCategoryName] = useState('Select Sub Folder');
  ...
  useEffect(() => {
    ...
    if (path != undefined && index != undefined) {
      ...
      newImages[index] = {
        ...
        filename: fileName,
        type: 'image/jpg',
        data:Platform.OS === 'android' ? ReactNativeBlobUtil.wrap(path) :
                  ...
                  anchor={
                    <TouchableOpacity
                      ...
                      <View
                        ...
                        color={
                          categoryName=='Select/Add Folder' ?COLORS.greylight : COLORS.secondary} />
                        <Text
                          style={{
                            color:categoryName=='Select/Add Folder' ?COLORS.greylight : COLORS.secondary,
                            marginLeft: '11%',
                          }}>
                          {categoryName}
                        </Text>
...
<View>
                  ...
                  anchor={
                    <TouchableOpacity
                      ...
                      <View
                        ...
                        <Icon name="folder" size={20} color={subCategoryName=='Select Sub Folder' ?COLORS.greylight : COLORS.secondary} />
                        <Text
                          style={{
                            color: subCategoryName=='Select Sub Folder' ?COLORS.greylight : COLORS.secondary,
                            marginLeft: '11%',
                          }}>
                          {subCategoryName}
                        </Text>
        ...
        <Appbar.Header
          ...
          <Appbar.Content
            ...
            // title={uniqId}
            title={cameraType == 'image' ? 'Take Photo' : 
            cameraType == 'doc' ? 'Scan PDF' : 
            'Choose Option'
          }
            // subtitle={}
          />
          ...
          <ScrollView
            ...
            {images.map((item, index) => (
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => {
                  setImage(item.data.replace('ReactNativeBlobUtil-file://', ''));
                  setVisible(true);
                }}
                key={index}>
                <ImageBackground
                  source={{uri:item.data.replace('ReactNativeBlobUtil-file://', '')}}
                  style={{
                    width: 80,
                    height: 80,
                    backgroundColor: COLORS.white,
        ...
        images.length == 5 ? (
          <View
            ...
            <Text
              ...
              You can only upload 5 images at a time. Please delete some images
            </Text>
          </View>
        ) : (
          <View
          ...
          <View
            ...
            <Text
              ...
              You can only upload 10 images at a time. Please delete some images
            </Text>
          </View>
        ) : (
          // camera screen
          ...
          <TouchableOpacity
            ...
            <Icon name="camera" size={34} color={COLORS.white} />
          </TouchableOpacity>
          {loading2 ? (
             <View
             style={{
